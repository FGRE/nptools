...
FILE* infile;
std::locale loc = generator().generate("ja_JP.CP932");
...
int main(int argc, char** argv)
{
    if (argc == 1)
        return 1;
    //TODO: Set this propertly because LAMENTO and LAMENTOTR require special handling
    NPAHead.gameid = 0;

    if (argc == 3 && strcmp(argv[2], "RECODE") == 0)
        RECODE = true;
    int i = 0, x = 0;
    ...
    NPAEntry = (_npaentry*)calloc(NPAHead.totalcount, sizeof(*NPAEntry));
    ...
    for(i = 0; i < NPAHead.totalcount; i++)
    {
        ...
        if (NPAEntry[i].type == 1)
            mkdir(utfPath.c_str(), 0777);
        else
            extractnpa(i, ftell(infile), (char*)utfPath.c_str());
    }
}
...
void extractnpa(int i, int pos, char* destination)
{
    ...
    if (!outfile)
    {
        printf("--WARNING: Cannot write file.");
        return;
    }
    ...
    if (NPAHead.compress == 1)
    {
        ...
        int szz = NPAEntry[i].origsize;
        uncompress((Bytef*)zbuffer,(uLongf*)&NPAEntry[i].origsize,buffer,NPAEntry[i].compsize);

        if (RECODE)
        {
            std::string recoded = to_utf<char>(zbuffer, zbuffer + szz, loc);
            fwrite(recoded.c_str(),1,recoded.size(),outfile);
        }
        else
